...
class MultistoreStorage extends StoreStorage {
  ...
  public function loadDefault() {
    ...
    if ($uuid) {
      // The parent class methods are colled here because apppropriate local
      // ones filter out all the stores which current user has no access to.
      $entity_query = parent::getQuery();
      $this->buildPropertyQuery($entity_query, ['uuid' => $uuid]);
